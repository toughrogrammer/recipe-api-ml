...
// class ContentBasedModel(val itemIds: BiMap[String, Int], val projection: DenseMatrix)
//   extends Serializable {
//   override def toString = s"Items: ${itemIds.size}"
// }

class ContentBasedAlgorithm(ap: CollaborativeAlgorithmParams)
//class ContentBasedAlgorithm(val ap: ContentBasedAlgorithmParams, val ap1: CollaborativeAlgorithmParams)
  // extends PAlgorithm if Model contains RDD[]
  extends CollaborativeAlgorithm(ap) {
...
*/
    ...
    val res = if(ap.normalizeProjection) {
      ...
      new CollaborativeModel(0, null, null, null, null, itemIds, normalizedProjection)
    } else {

      new CollaborativeModel(0, null, null, null, null, itemIds, projection)
    }
